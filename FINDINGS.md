...
- `scripts/fetch_pit_prices.py`：yfinance 抽埋退市股價格；關鍵兩步清洗 ——（a）**clip 落各自 membership 窗口**，剷走 yfinance _ticker-recycling_ junk（退市後個 symbol 派咗畀第二隻證券，e.g. TIE 退市後變咗交易喺 8000+ 嘅嘢）；（b）scrub 殘留單日 >±50-60% 嘅壞 print。最後 **634 隻成員有價（含 ~181 隻退市股入返測試）**。
...
## §12. 2026-06 新一輪：把 §11 + §10 兩條贏路疊埋（QQQ-CC + SMH 50/50 blend）

> 之前 §11[D] 證咗 QQQ/SMH 50/50 月度再平衡係無槓桿大幅 raw 贏嘅答案 (+5.0pp)；
> §10.2 證咗 QQQ + 5% OTM monthly covered call 係 risk-adjusted killer (Sharpe 1.5)，
> 連 QQQ 本身 + CC 都已經大幅 risk-adjusted 贏。
> **本節問：呢兩條疊埋係咪更強？再 extend SMH weight grid 同 3-way blend。**
>
> 工具：`scripts/research_qqq_smh_extensions.py`（4 個 fresh angles）+
> `scripts/research_qqq_smh_stress.py`（sub-period + cost 驗證）+
> `scripts/research_qqq_cc_split.py`（CC 只做 QQQ leg，SMH leg 原封不動嘅 realistic split）。

### 12.1 Fresh angles 結論

| Angle                                                        | 結果                                                      | 評                                            |
| ------------------------------------------------------------ | --------------------------------------------------------- | --------------------------------------------- |
| [4] SMH weight grid 10-90%                                   | 平滑 surface，10% 起就贏，越多越贏（80-90% 最贏但 DD 深） | 50/50 唔係 peak，係「舒服」嘅 choice          |
| [1] QQQ/SMH/GLD 3-way                                        | Q/S/G 40/40/20 輸 CAGR (+2.4pp)、贏 Sharpe/Calmar         | GLD 拖低 CAGR、加 risk-adjusted → 唔係 raw 路 |
| [2] QQQ/SMH 50/50 + 5% CC（整體 portfolio overlay，單一 IV） | h=0.9 → +14.8pp raw                                       | 理論 max，SMH 期權流動性假設緊                |
| [3] AIQ / BOTZ / ROBO 取代 SMH                               | 全部買-hold 同 blend 都輸 QQQ                             | SMH 嘅 edge 唔係 AI-broad，係 semis 集中      |
| **[5] QQQ-CC + SMH 50/50 split**（CC 淨係喺 QQQ leg）        | **h=0.9 → +7.5pp raw, MaxDD -32.8%（< QQQ -35.1%）**      | **🏆 現實可執行答案**                         |

### 12.2 [5] QQQ-CC + SMH 50/50 嘅 sub-period 穩定性（決定性）

| Sub-period | QQQ-CC h=0.9 + SMH 50/50 |   QQQ |     vs QQQ |
| ---------- | -----------------------: | ----: | ---------: |
| 2010-2014  |                    21.1% | 18.7% |  +2.4pp ✅ |
| 2015-2019  |                    22.4% | 16.7% |  +5.7pp ✅ |
| 2020-2026  |                    34.6% | 21.4% | +13.2pp ✅ |

**全部 sub-period 贏 raw**（包括 semis 弱期 2010-14 都有 +2.4pp cushion），同 §11[D] 嘅 sub-period robustness
一致。Sharpe 1.15、MaxDD -32.8%（仲低過 QQQ -35.1%）、Calmar 0.82 → **呢個組合係 "低 DD 兼高 return"**。

### 12.3 [5] QQQ-CC + SMH weight grid（h=0.9）

|       SMH weight |  CAGR | Sharpe |  MaxDD | vs QQQ |
| ---------------: | ----: | -----: | -----: | -----: |
| 0% (QQQ+CC only) | 26.5% |   1.26 | -31.5% | +7.3pp |
|              20% | 26.6% |   1.23 | -31.8% | +7.4pp |
|              50% | 26.8% |   1.15 | -32.8% | +7.5pp |
|              70% | 26.9% |   1.09 | -37.1% | +7.6pp |

**Weight surface 平**：SMH 0-70% 都係 26.5-27% range。CC 係主要 raw edge 來源（QQQ 本身加 CC = 26.5%），
SMH 加極少 CAGR（+0.1-0.3pp）但加深 DD。**保守選擇 = 20% SMH 配 80% QQQ-CC**（+7.4pp raw, DD 仲低過 QQQ）。

### 12.4 Haircut 壓力測試（最 honest 嘅敏感度）

|     Haircut | 50/50 + CC h= |  CAGR | Sharpe | vs QQQ |
| ----------: | ------------: | ----: | -----: | -----: |
| 1.0（理想） |               | 27.3% |   1.18 | +8.0pp |
|         0.9 |               | 26.8% |   1.15 | +7.5pp |
|        0.85 |               | 26.5% |   1.14 | +7.3pp |
|         0.8 |               | 26.3% |   1.13 | +7.1pp |

**Even at haircut 0.8（即係收到 80% theoretical premium）都仍然 +7.1pp raw 兼 Sharpe 1.13**。
呢個 robustness 遠好過 §10.2 嘅「QQQ + CC 單獨」（h=0.85 嗰陣就邊緣輸 QQQ），因為 SMH 嗰半做 raw
momentum 補返 CC 嗰半 cap 走嘅 upside。

### 12.5 對 goal 嘅 §12 結論

| 策略                                |      CAGR |   Sharpe |      MaxDD |     vs QQQ | 執行難度                        |
| ----------------------------------- | --------: | -------: | ---------: | ---------: | ------------------------------- |
| §11[D] QQQ/SMH 50/50 月度再平衡     |     24.2% |     1.03 |     -40.2% |     +5.0pp | 1（淨 ETF）                     |
| **§12[5] QQQ-CC + SMH 50/50**       | **26.8%** | **1.15** | **-32.8%** | **+7.5pp** | 2（要 monthly options rolling） |
| §12[5] aggressive (70% SMH)         |     26.9% |     1.09 |     -37.1% |     +7.6pp | 2                               |
| §12[2] theoretical max (CC on 整體) |     34.1% |     1.36 |     -34.6% |    +14.8pp | 3（SMH 期權假設流動性 OK）      |

**新增嘅「無槓桿大幅 raw 贏 QQQ」第三條路 = QQQ-CC + SMH blend**：

- **比 §11[D] 多 +2.5pp raw**（+7.5pp vs +5.0pp）、**DD 仲低過 QQQ 2.3pp**。
- 執行：每月底 roll 一次 QQQ 5% OTM 1-month call + 月度 QQQ/SMH 再平衡。SMH 嗰半唔做 CC。
- 現實 caveat：QQQ 5% OTM 月度 call premium 收到 ~80-90% theoretical（h=0.8-0.9），h=0.8 嗰陣仍 +7.1pp。
- 稅務 caveat：short term capital gain，稅後 CAGR 會再低 1-3pp（視乎 jurisdiction）。

**對 goal 嘅最終排名**（執行難度 ↑，CAGR ↑）：

1. **§9.2 QQQ/GLD 80/20 月度再平衡**：最簡單、最穩（無槓桿 risk-adjusted 贏，CAGR -1.5pp，Sharpe 1.03）。
2. **§11[D] QQQ/SMH 50/50 月度再平衡**：無 sector bet 接受、就淨 ETF → 24.2% raw（+5.0pp），DD -40%。
3. **§12[5] QQQ-CC + SMH 50/50 blend**：接受 options → 26.8% raw（+7.5pp），DD 仲低過 QQQ。
4. **§1-2 槓桿版（top5+QQQ ×1.15）**：傳統最強路 → 21.7% raw（+2.5pp，淨 low-DD），風險平過 QQQ。
...
## 12. Larry Williams 三規律 (SMA + %R + ROC) — 2026-07

腳本：`scripts/backtest_wpr_three_rules.py`  
Pine：`pine/larry_williams_three_rules_strategy.pine`  
副圖：`pine/larry_williams_three_rules_indicator.pine`

### 結論（先講）

| 版本                                                                 | 判斷                                                      |
| -------------------------------------------------------------------- | --------------------------------------------------------- |
| 原裝筆記 SMA50 + %R20 cross@-90 + ROC25 止賺 + partial 50%           | **唔能用**（指數 CAGR≈0 / 負）                            |
| 優化 long-only：SMA200 + %R(14)≤-85 level + ROC(20) 穿 MA10 且 ROC>0 | **risk-adjusted 可用**（Sharpe↑、DD 大減；CAGR 低過 B&H） |
| 做淡 / 單股 9988.HK                                                  | **唔 work**                                               |
| 動量後即 re-entry、%R 部分止賺                                       | 多數傷害或無益；預設關                                    |

定位：**防守型趨勢回調接貨**，唔係取代 QQQ 主策略。適合想「少回撤、勝率高、接受少啲報酬」嘅帳。

### 優化預設 (2005–2026, 0.05% 佣金, next-open)

| 標的    |  CAGR | Sharpe | MaxDD | 勝率 | 交易 | B&H CAGR | B&H Sh | B&H DD |
| ------- | ----: | -----: | ----: | ---: | ---: | -------: | -----: | -----: |
| SPY     |  5.9% |   0.88 |  -11% |  76% |   66 |    10.9% |   0.64 |   -55% |
| QQQ     |  7.5% |   0.83 |  -19% |  77% |   65 |    15.3% |   0.77 |   -53% |
| SMH     | 14.0% |   1.09 |  -18% |  66% |   68 |    19.2% |   0.74 |   -64% |
| 9988.HK | -9.6% |  -0.52 |  -58% |  27% |   11 |    -7.9% |   0.07 |   -80% |

Train≤2018 / Test≥2019：平均 te Sharpe ≈ **1.07**（best config），未見嚴重過擬合。

### 邊啲參數真係有用

1. **趨勢 filter 要用長均線（200）**：50 太短，假升勢入局太多。
2. **出場用 `roc_pos`**（ROC 穿下自身 MA **且 ROC 仍 >0**）：喺上升動量減弱時止賺，避免 ROC 已係負數先走（太遲）。
3. **`level` 入場**略勝 `cross`：升勢期間處於超賣區即可，唔一定要等穿線一刻。
4. **禁止 re-entry**：動量止賺後等 %R 離開超賣先再入，防抖。
5. **Partial %R 止賺**：勝率可以更高，但 CAGR/Sharpe 多數變差（截短贏家）。
6. **Short**：指數上無穩定 edge，Pine 預設關。

### 同主策略關係

- **唔取代** §1 動量 / vol-target 主線。
- 可作 **衛星帳** 或教學／單標的 risk-managed 模板。
- 要 raw CAGR 追 QQQ → 唔好用呢條；要細 DD + 高勝率 → 可以。
...
## 13. Weinstein 四階段 — 2026-07 優化

腳本：`scripts/backtest_weinstein_stage.py`  
Pine：`pine/weinstein_stage_strategy.pine`

### 結論

| 版本                                             | 判斷                                                   |
| ------------------------------------------------ | ------------------------------------------------------ |
| 原預設 MA150 / anytime / leave2 / 無緩衝         | 可用但 **QQQ 全期 DD -52%**（泡沬期 whipsaw）          |
| **優化 MA200 + 1% hysteresis + transition 入場** | **更可用**：QQQ DD **-52% → -24%**，test 2019+ Sh≈1.15 |
| stage4 撑頂                                      | CAGR 更高、DD 更大                                     |
| partial3 第三減倉                                | 勝率↑、波動↓、CAGR 略低                                |
| after_stage1 完整循環                            | 太嚴，報酬差                                           |
| 放量確認                                         | 指數有害，保持關                                       |
| 週線重採樣                                       | 未穩定贏日線近似                                       |

定位：中長線「只揸第二階段」趨勢工具；**risk-adjusted 贏 B&H，raw CAGR 多數輸長期牛市 B&H**。雙引擎 09- 仍然更強。

### 優化預設結果 (1999–2026, next-open, 5bps)

| 標的 |  CAGR | Sharpe |    MaxDD | 勝率 | 09+ Sh | B&H Sh | B&H DD |
| ---- | ----: | -----: | -------: | ---: | -----: | -----: | -----: |
| SPY  |  6.6% |   0.66 |     -20% |  40% |   0.84 |   0.53 |   -55% |
| QQQ  |  9.7% |   0.72 | **-24%** |  41% |   0.96 |   0.52 |   -83% |
| SMH  | 11.6% |   0.66 |     -34% |  30% |   0.80 |   0.47 |   -86% |

原 baseline QQQ MaxDD **-52%** → 優化 **-24%** 係今次最大改善。

### 有效改動

1. **MA 150→200**：略慢、假第二階段少啲
2. **1% hysteresis**：價要明顯企穩線上/跌穿線下先轉階段，減抖
3. **transition 入場**：只喺「剛進入第二」買，唔喺第二階段中段追入
4. leave2 出場仍然係預設（同原著「離開第二就減」一致）
...
## 14. 綜合最佳單標的策略 — 2026-07

文件：`BEST_STRATEGY.md`  
Pine：`pine/best_strategy.pine`  
回測：`scripts/backtest_best_composite.py`

### 結論

單標的（QQQ/SPY/SMH）技術策略 **冠軍 = 雙引擎調參版**：

- 突破 15/20 + MACD(5,35)>0
- RSI2 撈底：&lt;15 入 / &gt;75 出 + 200MA
- BO 止蝕 8%、MR 止蝕 4%
- 互斥、突破優先

打敗：舊雙引擎、+Chandelier、+stage/200MA on BO、Weinstein、%R 三規律、淨突破/淨 MR。

多資產「真主策略」仍然係 §1 vol-target 動量；無槓桿 risk-adj 仍然係 §9.2 QQQ/GLD。

---

## 15. SMH 跑贏 Buy&Hold — 2026-07

問題：雙引擎 SMH 09+ CAGR ~12% **大輸** 死揸 ~28%。

### 結論

| 做法                          | Full             | 09+    | 19+    | 判決             |
| ----------------------------- | ---------------- | ------ | ------ | ---------------- |
| 雙引擎 / 離場 TA              | 輸               | 輸     | 輸     | 空倉成本殺牛市   |
| 固定 1.5x                     | full 輸（2000s） | 贏     | 贏     | 唔穩             |
| **Vol-Target 35% cap2 vol40** | **贏**           | **贏** | **贏** | **唯一穩健答案** |

數字（5bps + 3% fin）：VT35 full 14.1% vs BH 10.8%；09+ 32.0% vs 28.2%；19+ 49.1% vs 42.8%。

Edge = 低波加槓桿，**唔係**選時離場。  
Pine：`pine/best_strategy.pine` 模式「SMH Vol-Target」。  
腳本：`scripts/backtest_smh_vol_target.py`。
